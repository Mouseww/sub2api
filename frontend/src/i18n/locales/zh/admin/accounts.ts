export default {
    accounts: {
      ...
      dataExportConfirmMessage: '导出的数据包含渠道与代理的敏感信息，请妥善保存。',
      dataExportConfirm: '确认导出',
      dataExported: '数据导出成功',
      dataExportedSkippedShadows: '数据已导出。已跳过 {count} 个 spark 影子渠道：其调度配置不在备份内，还原后需在重建的影子上重新调优。',
      dataExportFailed: '数据导出失败',
      dataImportTitle: '导入数据',
      dataImportHint: '上传导出的 JSON 文件以批量导入渠道与代理。',
      dataImportWarning: '导入将创建新渠道与代理，分组需手工绑定；请确认已有数据不会冲突。',
      dataImportFile: '数据文件',
      ...
      dataImportResultSummary: '代理创建 {proxy_created}，复用 {proxy_reused}，失败 {proxy_failed}；渠道创建 {account_created}，失败 {account_failed}',
      ...
      syncFromCrsTitle: '从 CRS 同步渠道',
      syncFromCrsDesc:
        '将 claude-relay-service（CRS）中的渠道同步到当前系统（不会在浏览器侧直接请求 CRS）。',
      crsVersionRequirement: '⚠️ 注意：CRS 版本必须 ≥ v1.1.240 才支持此功能',
      crsBaseUrl: 'CRS 服务地址',
      crsBaseUrlPlaceholder: '例如：http://127.0.0.1:3000',
      crsUsername: '用户名',
      crsPassword: '密码',
      syncProxies: '同时同步代理（按 host/port/渠道匹配或自动创建）',
      syncNow: '开始同步',
      syncing: '同步中...',
      ...
      crsExistingAccounts: '将自动更新的已有渠道',
      crsNewAccounts: '新渠道（可选择）',
      crsSelectAll: '全选',
      crsSelectNone: '全不选',
      crsNoNewAccounts: '所有 CRS 渠道均已同步。',
      crsWillUpdate: '将更新 {count} 个已有渠道。',
      crsSelectedCount: '已选择 {count} 个新渠道',
      ...
      schedulerScore: {
        ...
        stickyShort: '粘性',
        ungrouped: '未分组',
        hint: '显示格式为”分组名 / 基础分 / 粘性加分”。基础分按当前筛选条件限定的候选渠道计算，包含优先级、负载、排队、错误率、首包延迟、重置窗口、额度余量、计费倍率等因子；粘性加分只在开启粘性加权时用于 previous_response_id 或 session_hash。分数越大越优先。'
      },
      usageWindowsHint: '”5h / 7d”是上游渠道（如 OpenAI ChatGPT、Claude）官方的滚动用量窗口限制，由上游对渠道设定，并非 sub2api 配置，也与你映射的模型无关。窗口滚动到期后用量会自动重置，无法在 sub2api 端解除该限制。',
      ollamaCloud: {
      ...
      upstreamBilling: {
        ...
        autoProbeHint: '启用后按全局周期刷新上游声明倍率；此开关本身不会修改渠道倍率。',
        syncRate: '同步上游声明倍率',
        syncRateHint: '成功探测后自动更新渠道倍率，同步的是不含高峰的基准倍率；探测失败或声明超出允许范围时保持不变。开启本项会同时打开”自动探测上游声明倍率”。',
        syncRateManagedHint: '当前倍率由上游声明的基准倍率（不含高峰）自动维护。',
        syncedRateTooltip: '该渠道倍率由上游声明的基准倍率（不含高峰）自动同步',
      ...
      refreshToken: '刷新令牌',
      noAccountsYet: '暂无渠道',
      createFirstAccount: '添加上游渠道以开始使用 API 网关。',
