...
// HasCryptoDepositAddresses applies the HasEdge predicate on the "crypto_deposit_addresses" edge.
func HasCryptoDepositAddresses() predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
...
// HasCryptoDepositAddressesWith applies the HasEdge predicate on the "crypto_deposit_addresses" edge with a given conditions (other predicates).
func HasCryptoDepositAddressesWith(preds ...predicate.CryptoDepositAddress) predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := newCryptoDepositAddressesStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
