...
// The CryptoDepositAddressFunc type is an adapter to allow the use of ordinary function as a Querier.
type CryptoDepositAddressFunc func(context.Context, *ent.CryptoDepositAddressQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f CryptoDepositAddressFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.CryptoDepositAddressQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.CryptoDepositAddressQuery", q)
}

// The TraverseCryptoDepositAddress type is an adapter to allow the use of ordinary function as Traverser.
type TraverseCryptoDepositAddress func(context.Context, *ent.CryptoDepositAddressQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseCryptoDepositAddress) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseCryptoDepositAddress) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.CryptoDepositAddressQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.CryptoDepositAddressQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.CryptoDepositAddressQuery:
		return &query[*ent.CryptoDepositAddressQuery, predicate.CryptoDepositAddress, cryptodepositaddress.OrderOption]{typ: ent.TypeCryptoDepositAddress, tq: q}, nil
