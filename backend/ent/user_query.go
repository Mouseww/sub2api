...
type UserQuery struct {
	...
	withCryptoDepositAddresses *CryptoDepositAddressQuery
...
// QueryCryptoDepositAddresses chains the current query on the "crypto_deposit_addresses" edge.
func (_q *UserQuery) QueryCryptoDepositAddresses() *CryptoDepositAddressQuery {
	query := (&CryptoDepositAddressClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withCryptoDepositAddresses: _q.withCryptoDepositAddresses.Clone(),
...
// WithCryptoDepositAddresses tells the query-builder to eager-load the nodes that are connected to
// the "crypto_deposit_addresses" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithCryptoDepositAddresses(opts ...func(*CryptoDepositAddressQuery)) *UserQuery {
	query := (&CryptoDepositAddressClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withCryptoDepositAddresses = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		loadedTypes = [15]bool{
			...
			_q.withCryptoDepositAddresses != nil,
	...
	if query := _q.withCryptoDepositAddresses; query != nil {
		if err := _q.loadCryptoDepositAddresses(ctx, query, nodes,
			func(n *User) { n.Edges.CryptoDepositAddresses = []*CryptoDepositAddress{} },
			func(n *User, e *CryptoDepositAddress) {
				n.Edges.CryptoDepositAddresses = append(n.Edges.CryptoDepositAddresses, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadCryptoDepositAddresses(ctx context.Context, query *CryptoDepositAddressQuery, nodes []*User, init func(*User), assign func(*User, *CryptoDepositAddress)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(cryptodepositaddress.FieldUserID)
	}
	query.Where(predicate.CryptoDepositAddress(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.CryptoDepositAddressesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
